...
void afficherInfoTrou(typeTrou unTrou) {
    ...
    printf("Numero du trou : %d\n", unTrou.numero);
    printf("Distance du trou : %d\n", unTrou.distance);
    printf("Valeur du par : %d\n", unTrou.par);
    printf("Nombre maximum de coups possible : %d\n", unTrou.nbMaxCoups);
...
void afficherLesParcours(typeParcours *tab[], int nbParcours) {
    ...
    for (i = 0; i < NBMAXTROU; i++) {
        printf("Parcours %d\n '%s'", *tab[i]->nbTrou, *tab[i]->nom);
    }
...
void sauvegarderParcours(typeParcours *tab[], int nbParcours) {
    ...
    retour = fwrite(*tab, sizeof (typeParcours), nbParcours, fic);
    if (retour != nbParcours) {
        printf("%s\n", strerror(errno));
        exit(errno);
    }
