...
int main(int argc, char** argv) {
    ...
    fic = fopen("/home/USERS/ELEVES/SNIR2020/cmaillard/NetBeansProjects/Apprendre_C/Accès aux fichier/TD1/Ex4/essai2.txt", "w");
    if (fic == NULL) {
        printf("%s\n", strerror(errno));
        exit(errno);
    }
    
    //sauvegarde des 5personnes
    retour=fwrite(tabPersonne,sizeof (typePersonne),NBPERS,fic);
    if(retour!=NBPERS)
    {
        printf("%s\n", strerror(errno));
        exit(errno);
